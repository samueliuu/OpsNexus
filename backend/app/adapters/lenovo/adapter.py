...
@register_adapter("lenovo")
class LenovoAdapter(ServerAdapter):
    """Lenovo server adapter using XCC (XClarity Controller) Redfish API.

    References:
    - Lenovo python-redfish-lenovo (github.com/lenovo/python-redfish-lenovo)
    - Lenovo XCC Redfish API docs (lenovo.com/xcc-redfish)
    - Lenovo XCC3 Redfish API docs (newer ThinkSystem models)

    Lenovo XCC Redfish extensions:
    - Oem.Lenovo namespace for Lenovo-specific properties
    - FFDC (First Failure Data Capture) for diagnostic data
    - VPD (Vital Product Data) for hardware identification
    - XCC3 enhanced Redfish support on newer models
    - Lenovo-specific power/thermal policies
    """

    brand: str = "lenovo"
    supported_models: List[str] = [
        "ThinkSystem SR650", "ThinkSystem SR650 V2", "ThinkSystem SR650 V3",
        "ThinkSystem SR630", "ThinkSystem SR630 V2", "ThinkSystem SR630 V3",
        "ThinkSystem SR645", "ThinkSystem SR665",
        "ThinkSystem SR850", "ThinkSystem SR850 V2", "ThinkSystem SR860 V2",
        "ThinkSystem SD530", "ThinkSystem SD650", "ThinkSystem SD650 V3",
        "ThinkSystem ST550", "ThinkSystem ST650 V2",
    ]
    ...
    async def connect(self) -> None:
        ...
        await self._discover_lenovo_ids()

    async def disconnect(self) -> None:
        await self._redfish.disconnect()

    async def _discover_lenovo_ids(self) -> None:
        try:
    ...
    async def get_system_info(self) -> SystemInfo:
        ...
        try:
            oem = data.get("Oem", {})
            lenovo_oem = oem.get("Lenovo", {})
            if lenovo_oem:
                info.raw_data["lenovo_oem"] = lenovo_oem
        except Exception as e:
            logger.debug(f"Failed to get Lenovo OEM data from system: {e}")
        ...
        try:
            ...
            lenovo_mgr_oem = manager.get("Oem", {}).get("Lenovo", {})
            if lenovo_mgr_oem:
                info.raw_data["lenovo_manager_oem"] = lenovo_mgr_oem
        except Exception as e:
            logger.debug(f"Failed to get Lenovo XCC manager data: {e}")
    ...
    async def get_sensor_data(self) -> List[SensorData]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo thermal data: {e}")
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo power data: {e}")
    ...
    async def get_sel_logs(self, limit: int = 100) -> List[SELEntry]:
        ...
        log_urls = [
            f"{self._manager_path}/LogServices/StandardLog/Entries",
        ...
        for log_url in log_urls:
            try:
                ...
                logger.debug(f"Failed to get Lenovo log from {log_url}: {e}")
    ...
    async def get_firmware_inventory(self) -> List[FirmwareInfo]:
        ...
        try:
            ...
            for member in inventory.get("Members", []):
                ...
                if url:
                    try:
                        ...
                        lenovo_oem = fw_data.get("Oem", {}).get("Lenovo", {})
                        if lenovo_oem.get("UpdateStatus") == "Available":
                            update_status = "update_available"

                        firmware_list.append(FirmwareInfo(
                            component=fw_data.get("Name", ""),
                            current_version=fw_data.get("Version", ""),
                            available_version=lenovo_oem.get("AvailableVersion", ""),
            ...
            logger.warning(f"Failed to get Lenovo firmware inventory: {e}")
    ...
    async def get_network_adapters(self) -> List[NetworkAdapter]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo network adapters: {e}")
        return adapters
    ...
    async def get_storage_controllers(self) -> List[StorageController]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo storage controllers: {e}")
        return controllers
    ...
    async def get_power_supplies(self) -> List[PowerSupply]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo power supplies: {e}")
        return supplies
    ...
    async def get_fans(self) -> List[Fan]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo fans: {e}")
        return fans
    ...
    async def get_memory(self) -> List[MemoryModule]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo memory: {e}")
        return modules
    ...
    async def get_processors(self) -> List[Processor]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Lenovo processors: {e}")
        return processors

    async def get_xcc_info(self) -> Dict[str, Any]:
        try:
            manager = await self._redfish.get(self._manager_path)
            lenovo_oem = manager.get("Oem", {}).get("Lenovo", {})
            return {
                ...
                "xcc_type": lenovo_oem.get("ManagerType", "XCC"),
                "lenovo_oem": lenovo_oem,
                "raw_data": manager,
            }
        except Exception as e:
            logger.warning(f"Failed to get Lenovo XCC info: {e}")
            return {}

    async def get_ffdc_data(self) -> Dict[str, Any]:
        try:
            ffdc = await self._redfish.get(f"{self._manager_path}/FFDC")
            return {
                "enabled": ffdc.get("FFDCEnabled", True),
                "last_capture": ffdc.get("LastCaptureTime", ""),
                "raw_data": ffdc,
            }
        except Exception as e:
            logger.debug(f"Failed to get Lenovo FFDC data: {e}")
            return {}

    async def get_vpd_data(self) -> Dict[str, Any]:
        try:
            vpd = await self._redfish.get(f"{self._system_path}/Oem/Lenovo/VPD")
            return {
                "machine_type": vpd.get("MachineType", ""),
                "model": vpd.get("Model", ""),
                "serial": vpd.get("SerialNumber", ""),
                "product_id": vpd.get("ProductID", ""),
                "raw_data": vpd,
            }
        except Exception as e:
            logger.debug(f"Failed to get Lenovo VPD data: {e}")
            return {}

    async def get_bios_attributes(self) -> Dict[str, Any]:
        try:
            bios = await self._redfish.get(f"{self._system_path}/Bios")
            lenovo_oem = bios.get("Oem", {}).get("Lenovo", {})
            return {
                "current_boot_mode": bios.get("CurrentBootMode", ""),
                "attributes": bios.get("Attributes", {}),
                "lenovo_bios_settings": lenovo_oem.get("Bios", {}),
                "raw_data": bios,
            }
        except Exception as e:
            logger.debug(f"Failed to get Lenovo BIOS attributes: {e}")
            return {}
